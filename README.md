...
這份文件彙整了目前這個專案所有需要記住的資訊：專案背景、網址、金鑰、帳密、
部署設定、操作方式。請自行保管好這份文件，不要外流（裡面有實際可用的密鑰）。
...
### 下一階段方向
- 把目前使用的 Bot Token 換掉（因為過程中密鑰有曝光過，建議之後重新申請）
...
| DATABASE_URL | Neon 給的 PostgreSQL 連線字串 | 設定成 Render 的 Environment Variable，不要寫在 `main.py` 裡。實際值請到 [Neon Console](https://console.neon.tech) 查看 |
...
| Runtime | Python 3 |
| Environment Variables | `PYTHON_VERSION` = `3.11.9`<br>`DATABASE_URL` = Neon 的連線字串 |
| Instance Type | Free |
